...
# VM配置 - 500GB磁盘配置（输出目录改为windows_10）
VM_NAME="windows_10"
DISK_SIZE="512000"    # 500GB
...
create_startup_script() {
    ...
    cat > "${SCRIPT_DIR}/start-windows-vm.sh" << EOF
#!/bin/bash
# 启动生态系统应用VM
...
EOF
    ...
    chmod +x "${SCRIPT_DIR}/start-windows-vm.sh"
...
show_completion() {
    ...
    echo -e "${CYAN}如何使用:${NC}"
    echo "  1. 启动VM: ./start-windows-vm.sh"
    ...
    if [ -d "$output_dir" ]; then
        ...
        echo "  镜像文件: $(ls -lh "$output_dir"/${VM_NAME} 2>/dev/null | awk '{print $5}' || echo '未找到')"
    fi
    echo ""
    
    echo -e "${YELLOW}提醒: 根据PDF要求，需要将输出目录打包成ZIP文件提交给Philips${NC}"
    echo ""
}

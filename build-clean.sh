...
REM 安装Chocolatey包管理器 - 修复版本
echo [INFO] 安装Chocolatey包管理器...
powershell -NoProfile -ExecutionPolicy Bypass -Command "& {[System.Net.ServicePointManager]::SecurityProtocol = [System.Net.ServicePointManager]::SecurityProtocol -bor 3072; iex ((New-Object System.Net.WebClient).DownloadString('https://community.chocolatey.org/install.ps1'))}"

REM 等待Chocolatey安装完成并刷新环境
timeout /t 10 /nobreak
call refreshenv
set PATH=%PATH%;%ALLUSERSPROFILE%\chocolatey\bin

REM 验证Chocolatey安装
echo [INFO] 验证Chocolatey安装...
where choco >nul 2>&1
if %errorlevel% neq 0 (
    echo [ERROR] Chocolatey安装失败，尝试手动设置路径
    set PATH=%ALLUSERSPROFILE%\chocolatey\bin;%PATH%
)

REM 安装开发环境 - 分批安装避免超时
echo [INFO] 安装核心开发工具...
"%ALLUSERSPROFILE%\chocolatey\bin\choco.exe" install dotnet-6.0-sdk -y --timeout=1800
"%ALLUSERSPROFILE%\chocolatey\bin\choco.exe" install dotnet-8.0-sdk -y --timeout=1800
"%ALLUSERSPROFILE%\chocolatey\bin\choco.exe" install openjdk17 -y --timeout=1800
"%ALLUSERSPROFILE%\chocolatey\bin\choco.exe" install nodejs -y --timeout=1800
"%ALLUSERSPROFILE%\chocolatey\bin\choco.exe" install python -y --timeout=1800

echo [INFO] 安装开发工具...
"%ALLUSERSPROFILE%\chocolatey\bin\choco.exe" install vscode -y --timeout=1800
"%ALLUSERSPROFILE%\chocolatey\bin\choco.exe" install git -y --timeout=1800
"%ALLUSERSPROFILE%\chocolatey\bin\choco.exe" install 7zip -y --timeout=1800

echo [INFO] 安装浏览器...
"%ALLUSERSPROFILE%\chocolatey\bin\choco.exe" install googlechrome -y --timeout=1800
"%ALLUSERSPROFILE%\chocolatey\bin\choco.exe" install firefox -y --timeout=1800

echo [INFO] 安装数据库工具...
"%ALLUSERSPROFILE%\chocolatey\bin\choco.exe" install mysql.workbench -y --timeout=1800
"%ALLUSERSPROFILE%\chocolatey\bin\choco.exe" install pgadmin4 -y --timeout=1800

echo [INFO] 安装系统工具...
"%ALLUSERSPROFILE%\chocolatey\bin\choco.exe" install putty -y --timeout=1800
"%ALLUSERSPROFILE%\chocolatey\bin\choco.exe" install notepadplusplus -y --timeout=1800

REM 配置Git（使用完整路径）
echo [INFO] 配置Git环境...
if exist "C:\Program Files\Git\bin\git.exe" (
    "C:\Program Files\Git\bin\git.exe" config --global init.defaultBranch main
    "C:\Program Files\Git\bin\git.exe" config --global user.name "Ecosystem Developer"
    "C:\Program Files\Git\bin\git.exe" config --global user.email "developer@ecosystem.local"
) else (
    echo [WARNING] Git未找到，跳过配置
)

echo [SUCCESS] 生态系统工具安装完成！
echo [INFO] 已安装核心开发工具，其他工具可在系统启动后手动安装
EOF
